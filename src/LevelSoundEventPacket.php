...
class LevelSoundEventPacket extends DataPacket implements ClientboundPacket, ServerboundPacket{
	 ...
	 */
	public static function create(int $sound, Vector3 $position, int $extraData, string $entityType, bool $isBabyMob, bool $disableRelativeVolume, int $entityUniqueId = -1) : self{
		$result = new self;
		$result->sound = $sound;
	...
	public static function nonActorSound(int $sound, Vector3 $position, bool $disableRelativeVolume, int $extraData = -1, int $entityUniqueId = -1) : self{
		return self::create($sound, $position, $extraData, ":", false, $disableRelativeVolume, $entityUniqueId);
	}
	...
	protected function decodePayload(PacketSerializer $in) : void{
		...
		$this->entityUniqueId = $in->getVarLong();
	}
	...
	protected function encodePayload(PacketSerializer $out) : void{
		...
		$out->putVarLong($this->entityUniqueId);
	}
